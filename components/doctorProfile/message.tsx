...
function Message({senderType, senderName, time, text} : MessageParams) {
  ...
  return (
    <div className="flex items-start gap-2.5 mx-2 mt-4" dir={senderType==="doctor" ? "rtl" : "ltr"}>
      <Image className="w-8 h-8 rounded-full" src={userImage} alt={`${senderType } Image`} />
      <div className="flex flex-col gap-1 w-full max-w-[fit-content]">
          <div className="flex items-center space-x-2 rtl:space-x-reverse">
            <span className="text-sm font-semibold text-gray-900">{senderType==="doctor" ? `Dr. ${senderName}` : senderName}</span>
